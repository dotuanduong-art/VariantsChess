...
exports.PhantomVariant = void 0;
const Piece_1 = require("../../pieces/Piece");
const GhostHandler_1 = require("../../effect/handlers/GhostHandler");
const PossessionHandler_1 = require("../../effect/handlers/PossessionHandler");
const StealthMaintenanceHandler_1 = require("../../effect/handlers/StealthMaintenanceHandler");
const Board_1 = require("../../board/Board");
const apCostConfig_1 = require("../apCostConfig");
exports.PhantomVariant = {
    id: 'phantom',
    name: 'Phantom',
    ...
    effectHandlers: [
        new GhostHandler_1.GhostHandler(),
        new PossessionHandler_1.PossessionHandler(),
        new StealthMaintenanceHandler_1.StealthMaintenanceHandler(),
    ],
    getInitialState: () => ({}),
    passiveHooks: (state, player) => [
        {
            ...
            handler: (event, enqueueAction) => {
                ...
                if (!pieceSnapshot || !position)
                    return;
                // Trigger only if the dying piece is allied and carries the ghost effect
                if (pieceSnapshot.color === player && pieceSnapshot.effects?.some((e) => e.type === 'ghost')) {
                    let bestPiece = null;
                    ...
                    for (let r = 0; r < Board_1.BOARD_SIZE; r++) {
                        for (let c = 0; c < Board_1.BOARD_SIZE; c++) {
                            ...
                            if (p && p.id !== pieceSnapshot.id && p.color === player) {
                                if (p.type === Piece_1.PieceType.King || p.type === Piece_1.PieceType.Knight)
                                    continue;
                                if (p.effects?.some(e => e.type === 'ghost'))
                                    continue;
    ...
    skills: [
        {
            ...
            apCost: (state, player) => {
                ...
                return free > 0 ? 0 : apCostConfig_1.APCostConfig.phantom.phantom_skill_1;
            },
            cooldown: 0,
            ...
            canActivate(state, player, targets) {
                ...
                if (piece.type === Piece_1.PieceType.Knight) {
        ...
        },
        {
            ...
            tier: 'skill2',
            apCost: apCostConfig_1.APCostConfig.phantom.phantom_skill_2,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements: () => [],
            canActivate(state, player, targets) {
        ...
        },
        {
            ...
            tier: 'ultimate',
            apCost: apCostConfig_1.APCostConfig.phantom.phantom_ultimate,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements: () => [],
            canActivate(state, player, targets) {
...
//# sourceMappingURL=PhantomVariant.js.map