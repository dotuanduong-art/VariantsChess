...
exports.PirateVariant = void 0;
const Piece_1 = require("../../pieces/Piece");
const Board_1 = require("../../board/Board");
const apCostConfig_1 = require("../apCostConfig");
const SailingHandler_1 = require("../../effect/handlers/SailingHandler");
const PirateBetHandler_1 = require("../../effect/handlers/PirateBetHandler");
const Piece_2 = require("../../pieces/Piece");
exports.PirateVariant = {
    id: 'pirate',
    name: 'Pirate',
    ...
    effectHandlers: [
        new SailingHandler_1.SailingHandler(),
        new PirateBetHandler_1.PirateBetHandler(),
    ],
    getInitialState: () => ({}),
    ...
    skills: [
        // ── Skill 1: Bet (4 AP) ──
        {
            ...
            tier: 'skill1',
            apCost: apCostConfig_1.APCostConfig.pirate.pirate_skill_1,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements: () => [],
            canActivate(state, player, targets) {
                const ap = player === Piece_1.Color.White ? state.whiteAP : state.blackAP;
                if (ap < 0) {
        ...
        // ── Skill 2: Sailing (7 AP) ──
        {
            ...
            tier: 'skill2',
            apCost: apCostConfig_1.APCostConfig.pirate.pirate_skill_2,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements: () => [
                {
        ...
        // ── Ultimate: Broadside (12 AP) ──
        {
            ...
            tier: 'ultimate',
            apCost: apCostConfig_1.APCostConfig.pirate.pirate_ultimate,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements: () => [],
            canActivate(state, player, targets) {
            ...
            execute(state, player, targets) {
                ...
                const gridCopy = []; // piece ID or null
                ...
                for (let r = 0; r < Board_1.BOARD_SIZE; r++) {
                    for (let c = 0; c < Board_1.BOARD_SIZE; c++) {
                        ...
                        if (rook.color !== player || rook.type !== Piece_1.PieceType.Rook)
                            continue;
                        // Scan column forward from Rook's position
                        for (let targetRow = r + direction; targetRow >= 0 && targetRow < Board_1.BOARD_SIZE; targetRow += direction) {
                            const currentCellId = gridCopy[targetRow][c];
                            if (!currentCellId)
                                continue;
                            const targetMapping = pieceMap.get(currentCellId);
                            if (!targetMapping)
                                break; // Path blocked or invalid
                            ...
                            if (target.type === Piece_1.PieceType.King) {
                            ...
                            actions.push({
                                ...
                                capturedPieceSnapshot: { ...target, effects: target.effects ? target.effects.map((e) => ({ ...e })) : [] },
                                stayInPlace: true,
                            });
                            ...
                            for (let dr = -1; dr <= 1; dr++) {
                                for (let dc = -1; dc <= 1; dc++) {
                                    ...
                                    if (blastPos.col < 0 || blastPos.col >= Board_1.BOARD_SIZE || blastPos.row < 0 || blastPos.row >= Board_1.BOARD_SIZE)
                                        continue;
                                    ...
                                    if (blastMapping) {
                                        ...
                                        if (blastPiece.color === opponent && blastPiece.type !== Piece_1.PieceType.King) {
                                            actions.push({
                                                type: 'DESTROY_PIECE',
...
//# sourceMappingURL=PirateVariant.js.map