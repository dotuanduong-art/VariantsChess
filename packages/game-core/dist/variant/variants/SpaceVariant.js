...
exports.SpaceVariant = void 0;
exports.syncDimensionPortalCellEffects = syncDimensionPortalCellEffects;
const Piece_1 = require("../../pieces/Piece");
const Board_1 = require("../../board/Board");
const apCostConfig_1 = require("../apCostConfig");
function syncDimensionPortalCellEffects(state) {
...
function cleanupPortalOccupants(state, player) {
    const pairs = state.variantState.dimensionPairs || [];
    for (const pair of pairs) {
...
exports.SpaceVariant = {
    ...
    skills: [
        // ── Skill 1: Dimension Link (3 AP) ──
        {
            ...
            tier: 'skill1',
            apCost: apCostConfig_1.APCostConfig.space.space_skill_1,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements(state, player) {
                const ownHalfRegion = [];
                const opponentHalfRegion = [];
                const isWhite = player === Piece_1.Color.White;
                for (let r = 0; r < Board_1.BOARD_SIZE; r++) {
                    for (let c = 0; c < Board_1.BOARD_SIZE; c++) {
                        ...
                        if (isOwn)
                            ownHalfRegion.push(pos);
                        if (isOpp)
                            opponentHalfRegion.push(pos);
                    }
                }
                return [
                    {
        ...
        // ── Skill 2: Spatial Shift (4 AP) ──
        {
            ...
            tier: 'skill2',
            apCost: apCostConfig_1.APCostConfig.space.space_skill_2,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements(state, player) {
                return [{
            ...
            canActivate(state, player, targets) {
                ...
                const pairs = state.variantState.dimensionPairs || [];
                const matchedPair = pairs.find((p) => p.owner === player && ((p.odd.position.col === pos.col && p.odd.position.row === pos.row) ||
            ...
            execute(state, player, targets, rng) {
                const pos = targets[0].position;
                const pairs = state.variantState.dimensionPairs || [];
                const pair = pairs.find((p) => p.owner === player && ((p.odd.position.col === pos.col && p.odd.position.row === pos.row) ||
                    (p.even.position.col === pos.col && p.even.position.row === pos.row)));
                if (!pair)
                    return [];
                const actions = [];
                ...
                if (oddOccupant && (0, Piece_1.getPieceOwner)(oddOccupant) === opponentColor) {
                    actions.push({
                        type: 'DESTROY_PIECE',
                ...
                if (evenOccupant && (0, Piece_1.getPieceOwner)(evenOccupant) === opponentColor) {
                    actions.push({
                        type: 'DESTROY_PIECE',
        ...
        // ── Ultimate: Cosmic Void (9 AP) ──
        {
            ...
            tier: 'ultimate',
            apCost: apCostConfig_1.APCostConfig.space.space_ultimate,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements(state, player) {
                return [];
    ...
    passiveHooks: (state, player) => [
        ...
        },
        {
            ...
            handler: (event, enqueueAction) => {
                ...
                if (!capturedPieceSnapshot || capturedPieceSnapshot.color === player)
                    return;
...
//# sourceMappingURL=SpaceVariant.js.map