...
exports.PuppetVariant = void 0;
const Piece_1 = require("../../pieces/Piece");
const Board_1 = require("../../board/Board");
const PuppetNoCaptureHandler_1 = require("../../effect/handlers/PuppetNoCaptureHandler");
const PuppetControlHandler_1 = require("../../effect/handlers/PuppetControlHandler");
const PuppetTrapHandler_1 = require("../../effect/handlers/PuppetTrapHandler");
...
const MASTER_AP_MAP = {
    [Piece_1.PieceType.Pawn]: apCostConfig_1.APCostConfig.puppet.puppet_table.Pawn,
    [Piece_1.PieceType.Knight]: apCostConfig_1.APCostConfig.puppet.puppet_table.Knight,
    [Piece_1.PieceType.Bishop]: apCostConfig_1.APCostConfig.puppet.puppet_table.Bishop,
    [Piece_1.PieceType.Rook]: apCostConfig_1.APCostConfig.puppet.puppet_table.Rook,
    [Piece_1.PieceType.Queen]: apCostConfig_1.APCostConfig.puppet.puppet_table.Queen,
};
exports.PuppetVariant = {
    id: 'puppet',
    name: 'Puppet',
    description: 'Control your enemies, set traps, and redirect attacks to voodoo dolls.',
    effectHandlers: [
        new PuppetNoCaptureHandler_1.PuppetNoCaptureHandler(),
        new PuppetControlHandler_1.PuppetControlHandler(),
        new PuppetTrapHandler_1.PuppetTrapHandler(),
        new StunHandler_1.StunHandler(),
    ...
    passiveHooks: (state, player) => [
        // 1. Passive: OnEffectExpired — Chain Stun/Control expiration to Bind
        {
            ...
            handler: (event, enqueueAction) => {
                ...
                if (effectSnapshot.type === 'puppet_control' &&
        ...
        // 3. Passive: OnPieceDestroyed — If voodoo is destroyed, main effect vanishes
        {
            ...
            handler: (event, enqueueAction) => {
                ...
                if (pieceSnapshot.effects?.some((e) => e.type === 'voodoo')) {
    ...
    skills: [
        // ── Skill 1: Soul Binding (4 AP) ──
        {
            ...
            tier: 'skill1',
            apCost: apCostConfig_1.APCostConfig.puppet.puppet_skill_1,
            cooldown: 0,
        ...
        // ── Skill 2: Puppet Strings (3 AP) ──
        {
            ...
            tier: 'skill2',
            apCost: apCostConfig_1.APCostConfig.puppet.puppet_skill_2,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements(state, player) {
                return [
        ...
        // ── Ultimate: Puppet Master (Dynamic AP) ──
        {
            ...
            tier: 'ultimate',
            apCost: apCostConfig_1.APCostConfig.puppet.puppet_ultimate, // Base AP spent by core. Extra AP is spent during execute.
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements(state, player) {
                return [
            ...
            canActivate(state, player, targets) {
                ...
                if (targetPiece.type === Piece_1.PieceType.King) {
                ...
                // AP cost check
                const actualCost = MASTER_AP_MAP[targetPiece.type] || 2;
                const playerAP = player === Piece_1.Color.White ? state.whiteAP : state.blackAP;
            ...
            execute(state, player, targets) {
                ...
                const actualCost = MASTER_AP_MAP[piece.type] || 2;
...
//# sourceMappingURL=PuppetVariant.js.map