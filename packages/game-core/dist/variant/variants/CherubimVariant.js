...
exports.CherubimVariant = void 0;
 ...
 */
function cleanseAroundTotem(state, totemPos, totemColor, enqueueAction) {
...
exports.CherubimVariant = {
    ...
    onSetup(state, player) {
        ...
        if (!SpecialPieceRegistry_1.specialPieceRegistry.get('totem')) {
            SpecialPieceRegistry_1.specialPieceRegistry.register({
                id: 'totem',
                displayName: 'Totem',
                getLegalMoves: () => [],
            ...
            handler: (event, enqueueAction) => {
                ...
                for (let r = 0; r < Board_1.BOARD_SIZE; r++) {
                    for (let c = 0; c < Board_1.BOARD_SIZE; c++) {
                        ...
                        if (piece && piece.color === player && piece.type === Piece_1.PieceType.Pawn && piece.effects) {
                            ...
                            if (ascend) {
                                ...
                                if (ascend.metadata.appliedTurn === state.turnNumber &&
    ...
    skills: [
        // ── Skill 1: Ascend (2 AP) ──
        {
            ...
            tier: 'skill1',
            apCost: apCostConfig_1.APCostConfig.cherubim.cherubim_skill_1,
            cooldown: 0,
            ...
            canActivate(state, player, targets) {
                ...
                if (piece.type !== Piece_1.PieceType.Pawn) {
        ...
        // ── Skill 2: Fountain of Youth (5 AP) ──
        {
            ...
            tier: 'skill2',
            apCost: apCostConfig_1.APCostConfig.cherubim.cherubim_skill_2,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements(state, player) {
                return [{
            ...
            execute(state, player, targets, rng) {
                const pos = targets[0].position;
                const totemId = `totem_${player === Piece_1.Color.White ? 'w' : 'b'}_${rng.nextInt(0, 1000000)}`;
                return [
                    {
                        type: 'SPAWN_PIECE',
                        piece: {
        ...
        // ── Ultimate: Divine Ascension (12 AP) ──
        {
            ...
            tier: 'ultimate',
            apCost: apCostConfig_1.APCostConfig.cherubim.cherubim_ultimate,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements(state, player) {
                if (!state || !player)
            ...
            execute(state, player, targets, rng) {
                ...
                if (pawnsToPromote.length > 0) {
                    for (const item of pawnsToPromote) {
                        ...
                        item.piece.type = Piece_1.PieceType.Queen;
...
//# sourceMappingURL=CherubimVariant.js.map