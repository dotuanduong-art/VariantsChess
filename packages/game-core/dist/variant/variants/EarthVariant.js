...
function simulateShiftingPeaks(state, player, from, to) {
    ...
    for (const item of enemyPiecesOnPath) {
        ...
        while (stepsPushed < 3) {
            ...
            if (next.col < 0 || next.col >= Board_1.BOARD_SIZE || next.row < 0 || next.row >= Board_1.BOARD_SIZE) {
...
exports.EarthVariant = {
    ...
    onSetup(state, player) {
        if (!SpecialPieceRegistry_1.specialPieceRegistry.get('mountain')) {
            SpecialPieceRegistry_1.specialPieceRegistry.register({
                id: 'mountain',
                displayName: 'Mountain',
                getLegalMoves: () => [],
        ...
        if (!SpecialPieceRegistry_1.specialPieceRegistry.get('earth_reservation')) {
            SpecialPieceRegistry_1.specialPieceRegistry.register({
                id: 'earth_reservation',
                displayName: 'Earth Reservation',
                getLegalMoves: () => [],
        ...
        if (state.variantState.skill2CostThisTurn === undefined) {
            state.variantState.skill2CostThisTurn = apCostConfig_1.APCostConfig.earth.earth_skill_2;
        }
    ...
    getInitialState: () => ({
        skill2CostThisTurn: apCostConfig_1.APCostConfig.earth.earth_skill_2,
        shadowedPieces: [],
    }),
    passiveHooks: (state, player) => [
        {
            ...
            handler: (event) => {
                if (event.activePlayer !== player)
                    return;
                const count = (0, SpecialPieceRegistry_1.countSpecialPieces)(state.board, player, 'mountain');
                state.variantState.skill2CostThisTurn = Math.max(3, apCostConfig_1.APCostConfig.earth.earth_skill_2 - count);
            },
        },
        {
            ...
            handler: (event, enqueueAction) => {
                ...
                if (snapshot.type === 'mountain_timer') {
                    ...
                    if (found && found.piece.specialType === 'earth_reservation') {
                        ...
                        const idx = shadowed.findIndex((p) => p.reservationPieceId === reservationId);
                        if (idx !== -1) {
    ...
    skills: [
        // ── Skill 1: Raise the Mountain (3 AP) ──
        {
            ...
            tier: 'skill1',
            apCost: apCostConfig_1.APCostConfig.earth.earth_skill_1,
            cooldown: 0,
            ...
            execute(state, player, targets, rng) {
                const pos = targets[0].position;
                const mountainId = `mountain_${player === Piece_1.Color.White ? 'w' : 'b'}_${rng.nextInt(0, 1000000)}`;
                return [
                    {
                        type: 'SPAWN_PIECE',
                        piece: {
        ...
        // ── Skill 2: Shifting Peaks (6 AP base, min 3 AP) ──
        {
            ...
            tier: 'skill2',
            apCost: (state, player) => state.variantState.skill2CostThisTurn ?? apCostConfig_1.APCostConfig.earth.earth_skill_2,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements(state, player) {
                return [
            ...
            execute(state, player, targets, rng) {
                ...
                const mountainBId = `mountain_${player === Piece_1.Color.White ? 'w' : 'b'}_${rng.nextInt(0, 1000000)}`;
                actions.push({
                    type: 'SPAWN_PIECE',
                    piece: {
                        id: mountainBId,
                ...
                const mountainAId = `mountain_${player === Piece_1.Color.White ? 'w' : 'b'}_${rng.nextInt(0, 1000000)}`;
                actions.push({
                    type: 'SPAWN_PIECE',
                    piece: {
                        id: mountainAId,
        ...
        // ── Ultimate: Earth-Burst Heavenly Star (10 AP) ──
        {
            ...
            tier: 'ultimate',
            apCost: apCostConfig_1.APCostConfig.earth.earth_ultimate,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements(state, player) {
                return [{
            ...
            execute(state, player, targets, rng) {
                ...
                const reservationPieceId = `reservation_${player === Piece_1.Color.White ? 'w' : 'b'}_${rng.nextInt(0, 1000000)}`;
                actions.push({
                    type: 'SPAWN_PIECE',
                    piece: {
...
//# sourceMappingURL=EarthVariant.js.map