...
exports.DragonSentinelVariant = void 0;
const Piece_1 = require("../../pieces/Piece");
const Board_1 = require("../../board/Board");
const SubterraneanEscapeHandler_1 = require("../../effect/handlers/SubterraneanEscapeHandler");
const DragonsRoarBeamHandler_1 = require("../../effect/handlers/DragonsRoarBeamHandler");
const ShieldHandler_1 = require("../../effect/handlers/ShieldHandler");
const apCostConfig_1 = require("../apCostConfig");
function findKing(state, player) {
...
function getOrthogonalDirection(from, to) {
...
function fireBeam(state, player, enqueueAction) {
    const roar = state.variantState.dragonsRoar;
    if (!roar || roar.beamActive)
        return;
    ...
    for (let i = 1; i <= 15; i++) {
        ...
        if (col >= 0 && col < Board_1.BOARD_SIZE && row >= 0 && row < Board_1.BOARD_SIZE) {
            ...
            if (p && p.color !== player && p.type !== Piece_1.PieceType.King) {
                enqueueAction({
                    type: 'DESTROY_PIECE',
                    pieceId: p.id,
                    position: { col, row },
...
exports.DragonSentinelVariant = {
    id: 'dragon_sentinel',
    name: 'Dragon Sentinel',
    description: 'Defensive powerhouse protecting allies and channeling ultimate line-of-destruction.',
    effectHandlers: [
        new SubterraneanEscapeHandler_1.SubterraneanEscapeHandler(),
        new DragonsRoarBeamHandler_1.DragonsRoarBeamHandler(),
        ...
        // Passive: Shield closest ally to King every 2 rounds
        {
            ...
            handler: (event, enqueueAction) => {
                if (event.activePlayer !== Piece_1.Color.Black)
                    return; // Trigger only at round end
        ...
        // Ultimate: Sacrifice Reduction on Piece Destroyed
        {
            ...
            handler: (event, enqueueAction) => {
                ...
                if (roar && roar.active && !roar.beamActive && roar.casterColor === player) {
                    ...
                    if (pieceSnapshot && pieceSnapshot.color === player && pieceSnapshot.type !== Piece_1.PieceType.King) {
    ...
    skills: [
        // ── Skill 1: Subterranean Escape (4 AP) ──
        {
            ...
            tier: 'skill1',
            apCost: apCostConfig_1.APCostConfig.dragon_sentinel.dragon_sentinel_skill_1,
            cooldown: 0,
        ...
        // ── Skill 2: Shockwave (3 AP) ──
        {
            ...
            tier: 'skill2',
            apCost: apCostConfig_1.APCostConfig.dragon_sentinel.dragon_sentinel_skill_2,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements(state, player) {
                return [
            ...
            canActivate(state, player, targets) {
                ...
                for (let dr = -1; dr <= 1; dr++) {
                    for (let dc = -1; dc <= 1; dc++) {
                        ...
                        if (r >= 0 && r < Board_1.BOARD_SIZE && c >= 0 && c < Board_1.BOARD_SIZE) {
                            const p = state.board.getPiece({ col: c, row: r });
                            if (p && p.color !== player) {
                                hasEnemy = true;
                                break;
                ...
                for (let dr = -1; dr <= 1; dr++) {
                    for (let dc = -1; dc <= 1; dc++) {
                        ...
                        if (enemy && enemy.color !== player) {
                            ...
                            if (destCol >= 0 &&
                                destCol < Board_1.BOARD_SIZE &&
                                destRow >= 0 &&
                                destRow < Board_1.BOARD_SIZE &&
        ...
        // ── Ultimate: Dragon's Roar (10 AP) ──
        {
            ...
            tier: 'ultimate',
            apCost: apCostConfig_1.APCostConfig.dragon_sentinel.dragon_sentinel_ultimate,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements(state, player) {
                return [
...
//# sourceMappingURL=DragonSentinelVariant.js.map