...
exports.CannibalVariant = void 0;
...
exports.CannibalVariant = {
    ...
    skills: [
        // ── Skill 1: Royal Guard (3 AP) ──
        {
            ...
            tier: 'skill1',
            apCost: apCostConfig_1.APCostConfig.cannibal.cannibal_skill_1,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements: (state, player) => [{
                    type: 'piece',
                    filter: 'ally',
                    pieceType: Piece_1.PieceType.King,
            ...
            canActivate(state, player, targets) {
                ...
                if (!piece || piece.type !== Piece_1.PieceType.King || piece.color !== player) {
        ...
        // ── Skill 2: Devour (5 AP) ──
        {
            ...
            tier: 'skill2',
            apCost: apCostConfig_1.APCostConfig.cannibal.cannibal_skill_2,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements: (state, player) => [{
                    type: 'piece',
        ...
        // ── Ultimate: Apex Predator (8 AP) ──
        {
            ...
            tier: 'ultimate',
            apCost: apCostConfig_1.APCostConfig.cannibal.cannibal_ultimate,
            cooldown: 0,
            usageRule: 'once_per_turn',
            getTargetRequirements: (state, player) => [],
            canActivate(state, player, targets) {
                ...
                for (let r = 0; r < 15; r++) {
                    for (let c = 0; c < 15; c++) {
                        const p = state.board.getPiece({ col: c, row: r });
                        if (p && p.type === Piece_1.PieceType.King && p.color === player && p.effects) {
    ...
    passiveHooks: (state, player) => [
        ...
        },
        {
            ...
            handler: (event, enqueueAction) => {
                ...
                if (!effectSnapshot || effectSnapshot.sourcePlayer !== player)
                    return;
    ...
    moveModifiers: [
        {
            ...
            modify(moves, context) {
                ...
                if (piece.type !== Piece_1.PieceType.King)
                    return moves;
                const isCannibal = piece.color === Piece_1.Color.White ? state.whiteVariantId === 'cannibal' : state.blackVariantId === 'cannibal';
                if (!isCannibal)
                    return moves;
                const isUltimate = state.variantState.ultimateActive === true;
                if (isUltimate) {
                    const queenMoves = (0, MoveGenerator_1.getBaseMovesForType)(board, context.piecePosition, Piece_1.PieceType.Queen, piece.color, true);
                    const knightMoves = (0, MoveGenerator_1.getBaseMovesForType)(board, context.piecePosition, Piece_1.PieceType.Knight, piece.color, true);
                ...
                const moveType = state.variantState.kingCurrentMoveType || Piece_1.PieceType.King;
                return (0, MoveGenerator_1.getBaseMovesForType)(board, context.piecePosition, moveType, piece.color, true);
            }
        }
    ]
};
//# sourceMappingURL=CannibalVariant.js.map