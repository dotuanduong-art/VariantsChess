...
export class EnemySwapHandler implements EffectHandler {
  effectType = 'enemy_position_swap' as EffectType;
  ...
  handle(
    ...
    if (event.type === 'OnBeforePieceDestroyed') {
      ...
      const effect = pieceSnapshot.effects.find((e: Effect) => e.type === 'enemy_position_swap');
      if (!effect) return;
    ...
    if (event.type === 'OnEffectExpired') {
      ...
      if (!effectSnapshot || effectSnapshot.type !== 'enemy_position_swap') return;
