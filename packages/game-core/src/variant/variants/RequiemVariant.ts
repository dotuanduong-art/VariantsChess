...
export const RequiemVariant: VariantDefinition = {
  ...
  effectHandlers: [new FateHandler(), new BerserkHandler()],
  ...
  skills: [
    ...
    // Link 1 ally + 1 enemy with Fate effect, duration 3 rounds
    {
      ...
      canActivate(state, player, targets) {
        if (targets.length !== 2) return 'Select 1 ally and 1 enemy piece';
        ...
        for (let i = 0; i < 2; i++) {
          ...
          if (piece.type === PieceType.King) return 'Cannot target King';
        }
        ...
        if (!piece0 || piece0.color !== player) return 'First target must be an ally piece';
        if (!piece1 || piece1.color === player) return 'Second target must be an enemy piece';
    ...
    // Link 2 enemy pieces with Fate effect, duration 5 rounds
    {
      ...
      canActivate(state, player, targets) {
        if (targets.length !== 2) return 'Select 2 enemy pieces';
        ...
        for (let i = 0; i < 2; i++) {
          ...
          const piece = state.board.getPiece(t.position);
          if (!piece) return 'Piece not found';
          if (piece.color === player) return 'Must target enemy pieces';
          if (piece.type === PieceType.King) return 'Cannot target King';
        }
