...
export const VerdantDragonVariant: VariantDefinition = {
  id: 'verdant_dragon',
  name: 'Verdant Dragon',
  ...
  passiveHooks: (state: GameState, player: Color) => [
    // Hook 1: Dragon Counter Tick
    {
      ...
      handler: (event) => {
        if (event.type !== 'OnAPGained') return;
  ...
  skills: [
    // ── Skill 1 – Verdant Shelter (3 AP) ──
    {
      ...
      tier: 'skill1',
      apCost: APCostConfig.verdant_dragon.verdant_dragon_skill_1,
      cooldown: 0,
    ...
    // ── Skill 2 – Dragon\'s Gaze (4 AP) ──
    {
      ...
      tier: 'skill2',
      apCost: APCostConfig.verdant_dragon.verdant_dragon_skill_2,
      cooldown: 0,
      usageRule: 'once_per_turn',
      getTargetRequirements: () => [{
        type: 'piece',
    ...
    // ── Ultimate – Emerald Domain / Dragon\'s Wrath (Dynamic) ──
    {
      ...
      apCost: (state, player) => {
        ...
        return counter >= 100 ? APCostConfig.verdant_dragon.verdant_dragon_ultimate_wrath : APCostConfig.verdant_dragon.verdant_dragon_ultimate;
      },
      cooldown: 0,
      usageRule: 'once_per_turn',
