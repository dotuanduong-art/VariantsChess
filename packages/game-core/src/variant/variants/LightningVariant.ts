...
class ThunderTrapHandler implements EffectHandler {
  ...
  subscribesTo: GameEventType[] = ['OnMove'];
  ...
  handle(
    ...
    const trap = cellEffects.find(
      e => e.type === 'thunder_trap' && e.sourcePlayer !== piece.color
    );
...
export const LightningVariant: VariantDefinition = {
  id: 'lightning',
  name: 'Lightning',
...
function updateElectricTerrainState(state: GameState): void {
