...
import { Color, PieceType, Piece, oppositeColor } from '../../pieces/Piece';
...
function countSurvivingPieces(state: any): { white: number; black: number } {
  let white = 0;
  let black = 0;
  for (let r = 0; r < BOARD_SIZE; r++) {
    for (let c = 0; c < BOARD_SIZE; c++) {
      ...
      if (piece) {
        ...
        else if (piece.color === Color.Black) black++;
      }
    }
  }
  return { white, black };
...
function getVanguardTarget(
  state: any,
  player: Color,
  pos: Position
): { targetPiece: Piece; targetPos: Position } | null {
...
export const LordVariant: VariantDefinition = {
  id: 'lord',
  name: 'Lord',
  ...
  passiveHooks: (state, player) => [
    {
      ...
      handler: (event, enqueueAction) => {
        ...
        if (event.activePlayer !== Color.Black) return;
        if (event.turnNumber % 5 !== 0) return;
  ...
  skills: [
    // ── Skill 1: Vanguard Command (4 AP) ──
    {
      ...
      tier: 'skill1',
      apCost: APCostConfig.lord.lord_skill_1,
      cooldown: 0,
    ...
    // ── Skill 2: Reinforcements (4 AP) ──
    {
      ...
      tier: 'skill2',
      apCost: APCostConfig.lord.lord_skill_2,
      cooldown: 0,
      usageRule: 'once_per_turn',
      ...
      canActivate(state, player, targets) {
        ...
        if (targets.length === 2) {
          if (
            targets[0].position!.col === targets[1].position!.col &&
      ...
      execute(state, player, targets, rng): Action[] {
        const counts = countSurvivingPieces(state);
        const allyCount = player === Color.White ? counts.white : counts.black;
        const enemyCount = player === Color.White ? counts.black : counts.white;
        ...
        for (let i = 0; i < targets.length; i++) {
          ...
          const pawnId = `summoned_pawn_${player === Color.White ? 'w' : 'b'}_${rng.nextInt(0, 1000000)}_${i}`;
    ...
    // ── Ultimate: Iron Authority (9 AP) ──
    {
      ...
      tier: 'ultimate',
      apCost: APCostConfig.lord.lord_ultimate,
      cooldown: 0,
      usageRule: 'once_per_turn',
      getTargetRequirements: () => [],
      canActivate(state, player, targets) {
