...
import { ElectricHandler } from '../../effect/handlers/ElectricHandler';
import { ThunderFangHandler } from '../../effect/handlers/ThunderFangHandler';

// ─────────────────────────────────────────────────────────────────────────────
// Helper: find the player's King position
// ─────────────────────────────────────────────────────────────────────────────
function findKingPosition(state: Readonly<GameState>, player: Color) {
...
// ─────────────────────────────────────────────────────────────────────────────
function computeWrathZone(
  kingPos: { col: number; row: number },
  dirCol: number,
  dirRow: number
 ...
 */
function getPerpendicularOffsets(
  dc: number,
  dr: number
): { dc: number; dr: number }[] {
  if (dc === 0) {
...
function deriveDirection(
  ...
  if (absDc > 1 || absDr > 1 || (absDc === 0 && absDr === 0)) return null;
...
// ─────────────────────────────────────────────────────────────────────────────
export class ThunderFangCaptureValidator implements ActionValidator {
...
// ─────────────────────────────────────────────────────────────────────────────
export const ThunderDragonVariant: VariantDefinition = {
  id: 'thunder_dragon',
  name: 'Thunder Dragon',
  ...
  skills: [
    // ── Skill 1: Dragon's Scale (3 AP) ─────────────────────────────────────
    {
      ...
      name: "Dragon's Scale",
      description:
        "Grant an allied piece the Electric effect for 1 round. " +
        "If an enemy captures that piece, the attacker is stunned for 2 rounds.",
      tier: 'skill1',
      apCost: APCostConfig.thunder_dragon.thunder_dragon_skill_1,
      cooldown: 0,
    ...
    // ── Skill 2: Thunder Fang (6 AP) ────────────────────────────────────────
    {
      ...
      name: 'Thunder Fang',
      description:
        'Grant an allied piece Thunder Fang until end of current turn. ' +
      ...
      tier: 'skill2',
      apCost: APCostConfig.thunder_dragon.thunder_dragon_skill_2,
      cooldown: 0,
      usageRule: 'once_per_turn',
    ...
    // ── Ultimate: Dragon's Wrath (12 AP) ────────────────────────────────────
    {
      ...
      name: "Dragon's Wrath",
      description:
        "Choose a direction from your King (target any of the 8 adjacent squares). " +
      ...
      tier: 'ultimate',
      apCost: APCostConfig.thunder_dragon.thunder_dragon_ultimate,
      cooldown: 0,
      usageRule: 'once_per_turn',
        ...
        return [
          {
            ...
            region,
            description:
              "Select a cell adjacent to your King to set the blast direction",
          },
        ];
      },
